...
const CentralAvariaPage = () => {
  ...
  const fetchFinalizados = async (inicio?: string, fim?: string) => {
    try {
      ...
      const res = await api.get('/avaria/finalizados', { params });
      setAvarias(res.data);
    } catch (error) {
  ...
  const abrirDetalhe = async (id: string) => {
    try {
      setLoading(true);
      const av = avarias.find(a => a._id === id);
      const response = await api.get(`/avaria/${id}`);
      setDetalhe({
        ...response.data,
        visto_por_nome: av?.visto_por_nome || response.data.visto_por_nome,
        visto_data: av?.visto_data || response.data.visto_data,
        resolvido_por_nome: av?.resolvido_por_nome || response.data.resolvido_por_nome,
        resolvido_data: av?.resolvido_data || response.data.resolvido_data,
        resolvido_observacao: av?.resolvido_observacao || response.data.resolvido_observacao,
      });
    } catch (error) {
      console.error('Erro ao buscar detalhe:', error);
      alert('Erro ao carregar avaria');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: detalhe ? `Avaria_${formatDateShort(detalhe.data_snapshot)}` : 'Avaria',
  });
  ...
  const getFilteredItems = (): AvariaItemDetail[] => {
    if (!detalhe) return [];
    let items = detalhe.itens.filter(i => i.deposito_5 !== 0);
    ...
    if (statusFilter === 'contados') {
      items = items.filter(i => i.contagem !== null);
    } else if (statusFilter === 'com_divergencia') {
      items = items.filter(i => {
        if (i.contagem === null) return false;
        return Math.abs(Math.round((i.deposito_5 - i.contagem) * 100) / 100) > 0;
      });
    } else if (statusFilter === 'sem_divergencia') {
      items = items.filter(i => {
        if (i.contagem === null) return false;
        return Math.round((i.deposito_5 - i.contagem) * 100) / 100 === 0;
      });
    }

    if (sortDiferenca !== 'none') {
      items = [...items].sort((a, b) => {
        const da = a.contagem !== null ? Math.round((a.deposito_5 - a.contagem) * 100) / 100 : null;
        const db = b.contagem !== null ? Math.round((b.deposito_5 - b.contagem) * 100) / 100 : null;
  ...
  const calcularABC = (itens: AvariaItemDetail[]): Record<string, 'A' | 'B' | 'C'> => {
  ...
  const handleVisto = async () => {
    ...
    try {
      ...
      await api.put(`/avaria/${detalhe._id}/visto`);
      const response = await api.get(`/avaria/${detalhe._id}`);
  ...
  const handleResolvido = async () => {
    ...
    try {
      setActionLoading(true);
      await api.put(`/avaria/${detalhe._id}/resolvido`, { observacao: resolvidoObs });
      const response = await api.get(`/avaria/${detalhe._id}`);
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Central de Avaria</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-600 mb-2">Nenhuma avaria finalizada</h2>
            <p className="text-gray-500">Avarias finalizadas aparecerão aqui.</p>
          </div>
        ) : (
                ...
                {avarias.map((av) => {
                  ...
                  return (
                    <tr key={av._id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm text-gray-900">{formatDate(av.data_snapshot)}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{av.criado_por_nome}</td>
                      <td className="px-4 py-3 text-sm text-center font-medium">{av.total_itens}</td>
                      <td className="px-4 py-3 text-sm text-center text-green-600 font-medium">{av.itens_contados}</td>
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center space-x-3">
          ...
          <AlertTriangle className="text-red-600" size={28} />
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Relatório de Avaria <span className="text-red-600">(Depósito 5)</span></h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-lg shadow p-4 text-center">
          ...
          <p className="text-2xl font-bold text-green-600">{filteredItems.filter(i => i.contagem !== null).length}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 text-center">
          <p className="text-sm text-gray-500">Com Divergência</p>
          <p className="text-2xl font-bold text-orange-600">{filteredItems.filter(i => {
            if (i.contagem === null) return false;
            return Math.abs(Math.round((i.deposito_5 - i.contagem) * 100) / 100) > 0;
          }).length}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 text-center">
          ...
          <p className="text-2xl font-bold text-gray-600">{filteredItems.filter(i => {
            if (i.contagem === null) return false;
            return Math.round((i.deposito_5 - i.contagem) * 100) / 100 === 0;
          }).length}</p>
        </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase">Depósito 5</th>
              ...
              >
                <div className="flex items-center justify-end space-x-1">
                  ...
                  {sortDiferenca === 'asc' && <ArrowUp size={14} className="text-red-600" />}
                  {sortDiferenca === 'desc' && <ArrowDown size={14} className="text-red-600" />}
                </div>
              </th>
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredItems.length === 0 ? (
              ...
              const dif = item.contagem !== null ? Math.round((item.deposito_5 - item.contagem) * 100) / 100 : null;
              return (
                <tr key={item.codigo_item} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 text-sm text-right">{formatNumber(item.deposito_5)}</td>
                  <td className="px-3 py-2 text-sm text-right font-semibold">
                    {item.contagem !== null ? formatNumber(item.contagem) : <span className="text-gray-300">-</span>}
                  </td>
                  <td className="px-3 py-2 text-sm text-right font-semibold">
                    {dif !== null ? (
        ...
        <div className="px-4 py-3 bg-gray-50 text-sm text-gray-500">Exibindo {filteredItems.length} itens</div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <div className="bg-gray-50 p-4 rounded text-sm">
                ...
                <p>Contados: <strong>{filteredItems.filter(i => i.contagem !== null).length}</strong></p>
                ...
                return (
                  <tr key={item.codigo_item} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="border border-gray-300 px-2 py-1 text-right font-semibold">
                      {item.contagem !== null ? formatNumber(item.contagem) : '-'}
                    </td>
                    <td className={`border border-gray-300 px-2 py-1 text-right font-semibold ${
      ...
      {showResolvidoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            ...
            <p className="text-sm text-gray-600 mb-3">Adicione uma observação sobre a resolução desta avaria:</p>
            <textarea
              value={resolvidoObs}
              onChange={(e) => setResolvidoObs(e.target.value)}
              placeholder="Ex: Itens descartados, diferenças ajustadas..."
...
export default CentralAvariaPage;
